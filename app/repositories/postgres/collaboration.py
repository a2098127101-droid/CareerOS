...
class PostgresCollaborationRepository(SQLAlchemyRepo):
    ...
    def create_task(self,title:str,task_type:str,session_id:str="",tenant_id:str="demo-org",priority:str="normal",source:str="system",payload:dict|None=None,*,owner_user_id:str="")->dict:
    ...
    def update_task(self,task_id:str,status:str|None=None,priority:str|None=None,*,tenant_id:str|None=None)->dict:
        current=self.get_task(task_id,tenant_id=tenant_id);new_status=status if status is not None else current["status"];new_priority=priority if priority is not None else current["priority"]
        sql="UPDATE ai_tasks SET status=:status,priority=:priority,updated_at=CURRENT_TIMESTAMP WHERE task_id=:id";params={"status":new_status,"priority":new_priority,"id":task_id}
        if tenant_id is not None:sql+=" AND tenant_id=:tenant";params["tenant"]=tenant_id
