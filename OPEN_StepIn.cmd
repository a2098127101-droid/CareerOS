...
echo ================================================== > "%BOOTLOG%"
echo StepIn bootstrap - %date% %time% >> "%BOOTLOG%"
echo Root: %CD% >> "%BOOTLOG%"
echo ================================================== >> "%BOOTLOG%"

if exist ".venv\Scripts\python.exe" goto checkenv

echo StepIn first-run initialization...
...
echo.
echo Python 3.11+ is required for the full StepIn application.
echo The standalone compatibility showcase will open instead.
echo.
...
echo Installing StepIn dependencies...
...
if errorlevel 1 (
    echo Repairing StepIn dependencies...
    ".venv\Scripts\python.exe" -m pip install -r requirements.txt >> "%BOOTLOG%" 2>&1
    if errorlevel 1 goto fail
)
...
:fail
echo.
echo StepIn initialization failed.
echo Log: %BOOTLOG%
echo.
echo Opening the standalone compatibility showcase as a fallback.
start "" "CareerOS_H5_Showcase.html"
pause
exit /b 1
